...
This is an implementation of a currency conversion system involving 148 currencies, where each currency's rate is in relation to 1 US Dollar (USD).

The system operates based on these three files:
+ **Currency.java**, which represents the Currency object derived from deserialising the fx_rates.json file.
+ **User.java** represents the User object derived from deserialising the users.json file.
+ **Runner.java**, which demonstrates how the Currency and User java files interact with one another.
...
+ **users.json**, which contains a map of user profiles. Every *User* object has a **name** and a **wallet**. A **wallet** is a map of the *currency* to the value of the *currency* (Example: A user named 'Ali' has two currencies in his wallet: 25USD, and 14MYR).
...
2. Check if the *FROMCURRENCY* AND *TOCURRENCY* are both unique to one another and exists in the *fx_rates.json* file.
...
The message of every transaction will be stored in a logger file called logging.log. This file is located in the *src/main/logs* folder.
